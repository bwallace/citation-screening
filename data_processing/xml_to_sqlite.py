...
def pubmed_ids_to_db(pmids, db_path):
    ...
    d_to_sql(pmids_to_abs, db_path)    
                                            
    
def xml_to_sql(xml_path, db_path, ignore_these=[]):
    print "building a dictionary from %s..." % xml_path
    ref_d = xml_to_dict(xml_path)
    print "ok."
    d_to_sql(ref_d, db_path, ignore_these=ignore_these)

def d_to_sql(ref_d, db_path, ignore_these=[]):
    ...
    # now populate them
    to_sql(c, ref_d, ignore_these=ignore_these)

    print "success."
    conn.commit()
    conn.close()

def to_sql(c, xml_d, ignore_these=[]):
