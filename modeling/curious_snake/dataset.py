...
def line_to_instance(l, citation_screening_style=True):
    ''' 
    ...
    try:
        id = eval(l_split[0]) # first column is ID
        level_1_label = "?"
        if l_split[1] != "?":     
        ...
        print "hrmm.. something is wrong with this instance %s" % l_split
        pdb.set_trace()
    ...
    return instance(id, dict_point, level_1_label = level_1_label, level_2_label=level_2_label)
...
class dataset:
    ...
    def get_label_for_id(self, id):
        return self.instances[id].label
        
    def get_level_2_label_for_id(self, id):
        return self.instances[id].level_2_label
    ...
    def get_data_subset(self, inst_ids):
        ...
        for inst_id in inst_ids:
            examples.append(self.instances[inst_id])
        return dataset(examples)
