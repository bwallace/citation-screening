...
def grid_search(problem, param,  num_folds=5,
            ...
            MIN_G = -10, MAX_G = 3, G_STEP = 2, 
            beta = 10, sens_only=False, linear_kernel=False):
    ''' Re-implementation of grid search.'''
    ...
    # gamma is meaningless for linear kernels.
    if linear_kernel:
        print "grid_search: linear kernel is being used; not searching over gamma!"
        gamma_vals = [1]
    ...
    for C in C_vals:
        for gamma in gamma_vals:
            ...
            specificity = float(conf_mat["tn"]) / (float(conf_mat["tn"] + conf_mat["fp"]))
            metric = specificity + beta*sensitivity
            if sens_only:
                metric = sensitivity
